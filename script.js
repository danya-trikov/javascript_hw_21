...
const  Price = 300; // Початкова ціна товару
let quantity = prompt("Введiть кiлькiсть одиниць товару");
let discountCode = prompt("NEWYEAR"); 


// Визначення основної знижки
let discount = 0;
switch (discountCode) {
    case "NEWYEAR":
        discount = 20;
...
let totalPrice = initialPrice * quantity * (1 - discount / 100);

// Розрахунок додаткової знижки за кількість
if (quantity >= 3) {
    const additionalDiscount = 5;
    totalPrice *= (1 - addDiscount / 100);
}

// Розрахунок додаткової знижки за загальну суму покупки
if (totalPrice > 1000) {
